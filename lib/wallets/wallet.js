...
class Wallet {
    ...
    if (addresses.length > 0) {
      ...
      return bulk.execute((err) => {
        ...
        _.forEach(addressStringsChunks, (addressChunk) => {
          if (this._jobs) {
            this._jobs.addresses.fetchTransactions({ addresses: addressChunk, userId: self.userId, walletId: self._id })
          } else {
  ...
  _updateSingleAddresses (callback) {
    ...
    return this._mongo.db.collection('bitcoinaddresses').find({
      ...
      if (this._jobs) {
        this._jobs.addresses.fetchTransactions({ addresses: addressStrings, userId: self.userId, walletId: self._id })
      } else {
        log.warn('No jobs connection present. Will not create jobs for transaction fetching.')
      }
      return callback()
